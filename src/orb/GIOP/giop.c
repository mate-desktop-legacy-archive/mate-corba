 ...
 * use, we have to scan /tmp/matecorba-$USER-* to work out
...
giop_tmpdir_init (void)
{
...
#ifndef G_OS_WIN32
	env_dir = g_getenv("MATECORBA_SOCKETDIR");
	...
	dirname = g_strdup_printf ("matecorba-%s",
				   g_get_user_name ());
	while (!safe_dir) {
		char *newname;
		...
		if (iteration == 0)
			...
			MateCORBA_genuid_buffer ((guint8 *)&id, sizeof (id),
...
#ifndef G_OS_WIN32
	g_setenv ("MATECORBA_SOCKETDIR", safe_dir, TRUE);
#endif
	g_free (safe_dir);
	g_free (dirname);
}
...
#include "../orb-core/matecorba-policy.h"
...
first_valid_request (GIOPThread *tdata, gboolean *no_policy)
{
	...
	*no_policy = FALSE;
	...
	for (l = tdata->request_queue; l; l = l->next) {
		...
		MateCORBA_POAObject pobj = qe->poa_object;
