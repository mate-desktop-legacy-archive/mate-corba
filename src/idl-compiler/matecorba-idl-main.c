/**************************************************************************
    ...
    matecorba-idl-main.c (Driver program for the IDL parser & backend)
...
#define BASE_CPP_ARGS "-D__MATECORBA_IDL__ "
...
cl_version_callback (const char *option_name,
		     ...
		     GError **error)
{
  g_print ("matecorba-idl-2 %s - serial %d\n",
           VERSION, MATECORBA_CONFIG_SERIAL);
  exit(0);
}
...
int main(int argc, char *argv[])
{
  ...
  cl_cpp_args = g_string_new("-D__MATECORBA_IDL__ ");
   ...
   * with previous popt based versions of matecorba-idl-2
   */
  ...
  printf ("matecorba-idl-2 " VERSION " compiling\n");
  ...
  for (i = 0; idl_files[i]; ++i) {
    ...
    if (!matecorba_idl_to_backend(idl_files[i], &rinfo)) {
