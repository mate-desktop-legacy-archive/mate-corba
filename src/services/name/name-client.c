...
Usage (gchar * name)
{
  g_printerr
    ("Usage:\n"
     ...
     "       * '--mate' - the Mate Name Server is used\n"
...
Exception (CORBA_Environment * ev)
{
  switch (ev->_major)
      ...
      if (IS_EXCEPTION (ev, CosNaming_NamingContext_NotFound))
	{
	  ...
	  CORBA_char *name = 
	    MateCORBA_CosNaming_NameComponent_to_string (ex->rest_of_name._buffer);
	 ...
	 CORBA_Environment * ev)
{
  ...
  if (is_ext_context)
    ...
    {
      CosNaming_Name *name = MateCORBA_string_to_CosNaming_Name (str, ev);
      Exception (ev);
      if (name->_length != 0)
	{
  ...
  if (strcmp (argv[1], "--mate") == 0)
    {
      ...
      FILE *infile = popen ("xprop -root MATE_NAME_SERVER |"
			    "awk '{print $5}' |"
			    "xargs xprop MATE_NAME_SERVER_IOR -id |"
			    "sed 's/^[^\"]*\"//;s/\"//'", "r");
      fgets (objref_str, 2048, infile);
      pclose (infile);

      infile = popen ("xprop -root MATE_SESSION_CORBA_COOKIE |"
		      "sed 's/^[^\"]*\"//;s/\"$//;s:\\\\\\(.\\):\\1:g'", "r");
      ...
      MateCORBA_set_default_principal (&request_cookie);
#endif
    }
  ...
  if (argc > 3)			/* we have a name on argv[ 3 ] */
      ...
      if (mode != mode_destroy && mode != mode_list && mode != mode_resolve)
	{
	  if (is_ext_context)
	    ...
	    {
	      name = MateCORBA_string_to_CosNaming_Name (requested_name, &ev);
	      Exception (&ev);
	    }
	}
    }
	...
	while (TRUE)
	  {
		...
		CORBA_char *name = 
		  MateCORBA_CosNaming_NameComponent_to_string (comp);
